...
const pathwayPill = document.getElementById('pathwayPill');
...
async function runAnalysisOnce(ticker) {
  ...
  appendBot(html);
  // Background ingest into Pathway service (non-blocking)
  try {
    const docs = [];
    const headlines = (sent && sent.headlines) || [];
    headlines.forEach((h, i) => {
      docs.push({ id: `H${i+1}`, title: h.title || '', text: h.title || '', source: h.source || 'news', published_at: h.published_at || nowIso() });
    });
    const posts = (risk && risk.social_posts) || [];
    posts.forEach((p, i) => {
      docs.push({ id: `S${i+1}`, title: (p.text||'').slice(0,140), text: p.text || '', source: p.platform || 'social', published_at: p.published_at || nowIso() });
    });
    if (docs.length) tryPathwayIngest(ticker, docs);
  } catch (_) { /* ignore ingest errors */ }
  try {
...
async function ragAnswerFromContext({ question, sentiment, risk, k=5 }) {
  ...
  // Try remote Pathway first
  let remote = await tryPathwayQuery(question, ticker, k);
  if (remote && remote.length) {
    const top = remote.map((r, i) => ({ doc: { id: r.id, text: r.text, meta: { type: r.source && (r.source === 'twitter' || r.source === 'reddit' || r.source === 'stocktwits' ? 'social' : 'headline'), source: r.source, platform: r.source, published_at: r.published_at, title: r.title } }, score: r.score }));
    ...
    <div class="meta">Retriever: Pathway vector search</div>
    <div class="hr"></div>
    <div class="meta"><strong>Sources</strong></div>
    <div class="meta">${citationsHtml}</div>
    `;
    return { answerHtml: html, top, usedEmb: false, answer };
  }
  // Remote fallback to local embeddings
...
if (askBtn) {
  askBtn.addEventListener('click', async () => {
    ...
    const retriever = (pathwayPill && pathwayPill.textContent.includes('On')) ? 'pathway' : (ans.usedEmb ? 'embeddings' : 'tfidf');
    lastAnswerPayload = { question: q, retriever, ranked: (ans.top||[]).map((t,i)=>({ idx:i+1, id:t.doc.id, score:t.score, meta:t.doc.meta, text:t.doc.text })), answer: ans.answer };
  });
}
...
const pathwayUrl = document.getElementById('pathwayUrl');
...
function openSettings() {
  ...
  pathwayUrl.value = window.__settings.pathwayUrl || '';
...
saveSettingsBtn.addEventListener('click', () => {
  window.__settings = { newsUrl: newsUrl.value.trim(), socialUrl: socialUrl.value.trim(), pathwayUrl: pathwayUrl.value.trim(), pollInterval: Math.max(5, parseInt(pollInterval.value, 10) || 30) };
  saveSettings(window.__settings);
  appendBot('Settings saved.');
  closeSettings();
  // Try health check to flip pill
  if (window.__settings.pathwayUrl) { healthcheckPathway(window.__settings.pathwayUrl).then(ok=>setPathwayPill(!!ok)); }
});
...
window.addEventListener('load', () => {
  ...
  setLiveLabel();
  if (window.__settings && window.__settings.pathwayUrl) { healthcheckPathway(window.__settings.pathwayUrl).then(ok=>setPathwayPill(!!ok)); }
});

// ---------- Pathway integration ----------
async function healthcheckPathway(baseUrl) {
  try {
    const ctl = new AbortController();
    const to = setTimeout(()=>ctl.abort(), 1500);
    const res = await fetch(baseUrl.replace(/\/$/, '') + '/health', { signal: ctl.signal, cache: 'no-store' });
    clearTimeout(to);
    if (res.ok) return true; return false;
  } catch (_) { return false; }
}

function setPathwayPill(on) {
  if (!pathwayPill) return;
  if (on) { pathwayPill.textContent = 'Pathway: On'; pathwayPill.style.background = '#e6fffa'; pathwayPill.style.color = '#064e3b'; }
  else { pathwayPill.textContent = 'Pathway: Off'; pathwayPill.style.background = '#eee'; pathwayPill.style.color = '#555'; }
}

async function tryPathwayQuery(question, ticker, k=5) {
  const base = window.__settings && window.__settings.pathwayUrl;
  if (!base) { setPathwayPill(false); return null; }
  const url = base.replace(/\/$/, '') + '/query';
  try {
    const ctl = new AbortController();
    const to = setTimeout(()=>ctl.abort(), 2000);
    const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ticker, question, k }), signal: ctl.signal });
    clearTimeout(to);
    if (!res.ok) { setPathwayPill(false); return null; }
    const data = await res.json();
    if (!data || !Array.isArray(data.chunks)) { setPathwayPill(false); return null; }
    setPathwayPill(true);
    return data.chunks;
  } catch (e) {
    setPathwayPill(false);
    return null;
  }
}

async function tryPathwayIngest(ticker, docs) {
  const base = window.__settings && window.__settings.pathwayUrl;
  if (!base) return;
  const url = base.replace(/\/$/, '') + '/ingest';
  try {
    fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ticker, docs }) }).catch(()=>{});
  } catch (_) {}
}
